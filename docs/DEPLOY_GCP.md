...
アプリケーションが使用するポート (80, 8080, 8000 など) を許可する必要があります。
...
3. 設定例:
   ...
   - **プロトコルとポート**: `tcp:8080`, `tcp:8000`, `tcp:5173` (開発用サーバーの場合)
     - ※ 本番運用でNginx (ポート80) を使う場合、デフォルトのHTTP許可ルールで十分な場合があります。今回の `docker-compose.yml` ではフロントエンドをポート 5173:80 で公開しているため、外部からは 5173 でアクセスするか、80:80 にマッピングを変更することを推奨します。
...
4. 動作確認
   ブラウザで `http://<External-IP>:5173` (または設定したポート) にアクセスして確認します。
